"""
versao_3 — versao_2 + blefe all-in randômico.

Base idêntica ao versao_2 (equity tabular pré-flop + Monte Carlo pós-flop,
decisão por pot odds). A diferença: a cada decisão tem ~3% de chance de
ignorar a lógica de equity e dar all-in, desde que:

  - A mão não tenha equity alta (seria value bet, não blefe).
  - Ainda exista stack para dar shove (to_call < my_chips).

A ideia é introduzir variância e tornar o bot menos previsível: oponentes
que tentarem ler nosso range a partir de tamanho de aposta vão ver
ocasionalmente uma jogada que não bate com a lógica de equity.
"""
...
class Versao3(Player):

    MC_BUDGET_MS = 30.0
    MIN_SAMPLES = 20

    # Frequência do blefe all-in (por decisão)
    BLUFF_FREQ = 0.03
    # Equity máxima para considerar um shove como "blefe" (acima disso vira value)
    BLUFF_MAX_EQUITY = 0.55
    ...
    def decision(self, gv: GameView) -> int:
        # Calcula equity uma vez só
        if len(gv.board) == 0:
            equity = self._preflop_equity(gv.my_hand)
        else:
            equity = self._monte_carlo_equity(gv.my_hand, gv.board)

        # Blefe all-in randômico: mão fraca + stack para dar shove + roleta
        if (
            equity < self.BLUFF_MAX_EQUITY
            and gv.to_call < gv.my_chips
            and self._rng.random() < self.BLUFF_FREQ
        ):
    ...
    def _shove(self, gv: GameView) -> int:
        """Blefe all-in: aposta total = (já investido) + (todo o stack)."""
        invested = gv.current_bet - gv.to_call
        return invested + gv.my_chips


def create_player() -> Player:
    return Versao3("versao_3", Hand(), 0)
