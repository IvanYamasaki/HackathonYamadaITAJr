"""
versao_12 — v11 + counter-exploits dos alvos v11 e v1 (player_no_name).

Mudanças sobre a base v11 (validadas por sweep, 3000-4000 partidas/config):

  - VS_DISC_MULT 1.50: desconto 50% maior ao enfrentar agressão real.
    Os raises do v11 são quase só valor quando não overfoldamos cedo (o
    steal adaptativo dele cai ao piso 0.10 do ftr_scale) e os do v1 são
    valor puro (monster/strong) — pagar mais apertado contra eles é o
    grosso do ganho (+5-7pp sobre o clone).
  - ALLIN_DISC_ADD 0.04: mesma lógica na defesa de all-in (+1.5pp vs
    v11; neutro vs v1, que quase nunca shova).
  - R1_SB_CALL_HS 0.44 (era 0.40): SB mais tight na R1 (+2pp vs ambos).
    Nota: 0.44 custava -9pp vs v8 — este knob é específico do alvo.
  - O que NÃO funcionou no sweep: boost de bluff s4/s5 (49.8-50.5% vs
    v11, o ftr_scale já amplifica o suficiente), river value raise 0.70,
    SHOVE_HS 0.58, VS_DISC_MULT 0.85, R1_SB_CALL_HS 0.36.
...
class Versao12(Player):
    ...
    VS_DISC_MULT = 1.50       # multiplicador do desconto em _vs_raise (anti v11/v1)
...
def create_player() -> Player:
    return Versao12("versao_12", Hand(), 0)
